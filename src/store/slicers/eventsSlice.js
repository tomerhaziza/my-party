...
import { fetchAddNewEvent, fetchAllEvents, fetchRemoveEventById, updateEventById } from 'store/asyncThunk';
...
export const eventsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [updateEventById.fulfilled]: (state, action) => {
      ...
      const eventToUpdateIndex = newEventsList.findIndex((event) => event.id === eventId);
    ...
    [fetchRemoveEventById.fulfilled]: (state, action) => {
      const eventToRemoveIndex = state.events.findIndex((item) => item.id.toString() === action.payload.toString());
...
export const { updateLoginStatus } = eventsSlice.actions;
